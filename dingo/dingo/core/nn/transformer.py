...
from torch.nn import TransformerEncoder, TransformerEncoderLayer as PyTorchTransformerEncoderLayer

from dingo.core.nn.resnet import DenseResidualNet, MLP, LinearLayer
from dingo.core.utils import torchutils

class TransformerEncoderLayer(PyTorchTransformerEncoderLayer):
    """
    Custom TransformerEncoderLayer with gradient debugging print statements.
    Inherits from torch.nn.TransformerEncoderLayer and adds monitoring before linear1 and linear2.
    """
    ...
    def forward(
        ...
        """
        Forward pass with gradient debugging.
        """
        ...
        # Feed-forward block with debugging
        if self.norm_first:
            x_pre_ff = x
            x = x + self._ff_block_debug(self.norm2(x))
        else:
            x_pre_ff = x
            x = self.norm2(x + self._ff_block_debug(x))

        return x

    def _ff_block_debug(self, x: Tensor) -> Tensor:
        """
        Feed-forward block with debugging prints before linear1 and linear2.
        """
        # Linear 1 (expansion)
        x_ff_linear1 = self.linear1(x)

        # Debug print before linear1 output
        if not torch.isfinite(x_ff_linear1).all():
            print("\n========== BAD linear1 OUTPUT ==========")
            print("Contains NaN:", torch.isnan(x_ff_linear1).any().item())
            print("Contains Inf:", torch.isinf(x_ff_linear1).any().item())
            print("Max abs value:", x_ff_linear1.abs().max().item())
            print("Min value:", x_ff_linear1.min().item())
            print("Max value:", x_ff_linear1.max().item())
            print("========================================")
        """ else:
            print(
                f"linear1: mean={x_ff_linear1.mean().item():.6f}, "
                f"std={x_ff_linear1.std().item():.6f}, "
                f"max_abs={x_ff_linear1.abs().max().item():.6f}"
            ) """

        # Activation and dropout
        x = self.dropout1(self.activation(x_ff_linear1))

        # Linear 2 (projection back)
        x_ff_linear2 = self.linear2(x)

        # Debug print before linear2 output
        if not torch.isfinite(x_ff_linear2).all():
            print("\n========== BAD linear2 OUTPUT ==========")
            print("Contains NaN:", torch.isnan(x_ff_linear2).any().item())
            print("Contains Inf:", torch.isinf(x_ff_linear2).any().item())
            print("Max abs value:", x_ff_linear2.abs().max().item())
            print("Min value:", x_ff_linear2.min().item())
            print("Max value:", x_ff_linear2.max().item())
            print("========================================")
        """ else:
            print(
                f"linear2: mean={x_ff_linear2.mean().item():.6f}, "
                f"std={x_ff_linear2.std().item():.6f}, "
                f"max_abs={x_ff_linear2.abs().max().item():.6f}"
            ) """

        return self.dropout2(x_ff_linear2)
...
def create_transformer_enet(
    ...
    if tokenizer_kwargs is not None:
        ...
        #"""         
        tokenizer_kwargs = copy.deepcopy(tokenizer_kwargs)

        # Remove Beyond-GR configuration keys that are not Tokenizer arguments
        tokenizer_kwargs.pop("beyond_gr", None)
        #"""
