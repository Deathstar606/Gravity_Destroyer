...
def get_standardization_dict(
    ...
    if additional_parameters:
        ...
        samples = {p: np.empty(num_samples) for p in additional_parameters} #Additional parameters: ['beta_residual', 'beta_proxy', 'chi1z', 'chi2z']
        for n in range(num_samples):
            sample = {"parameters": wfd.parameters.iloc[n].to_dict()}
            sample = transform(sample)
            for p in additional_parameters:
                if "extrinsic_parameters" in sample and p in sample["extrinsic_parameters"]:
                    #print("SOURCE🫷", p, sample["extrinsic_parameters"][p])
                    samples[p][n] = sample["extrinsic_parameters"][p]

                elif "parameters" in sample and p in sample["parameters"]:
                    #print("SOURCE💦", p, sample["parameters"][p])
                    samples[p][n] = sample["parameters"][p]

                else:
                    raise KeyError(
                        f"Parameter '{p}' not found.\n"
                        f"Available parameter keys: "
                        f"{list(sample.get('parameters', {}).keys())}\n"
                        f"Available extrinsic keys: "
                        f"{list(sample.get('extrinsic_parameters', {}).keys())}"
                    )
