...
class BasePosteriorModel(ABC):
    ...
    def load_embedding_weights_only(self, model_filename: str, device: str = "xpu"):
        """
        Load weights from a saved posterior model, EXCLUDING the original normalizing flow weights.
        This is used to load shared feature extraction components (Transformer, Tokenizer, etc.)
        while initializing a new 2D flow from scratch.
        """
...
def train_epoch(
    ...
    for batch_idx, data in enumerate(dataloader): #the elusive dataloader
            ...
            for name, p in pm.network.named_parameters():
                ...
                if not torch.isfinite(grad).all():
                    print(
                        ...
                        f"Contains NaN : {torch.isnan(grad).any().item()}\n"
                        f"Contains Inf : {torch.isinf(grad).any().item()}\n"
                        f"Grad max: {grad.abs().max().item()}"
                    )
        ...
        # Debug 2: Periodically check gradient norm for the last transformer block
        if pm.rank is None or pm.rank == 0:
            try:
                current_iter = pm.iteration + loss_info.get_iteration()
            except Exception:
                current_iter = batch_idx
            if current_iter % 300 == 0:
                if getattr(pm.network, "embedding_net", None) is not None:
                    for name, param in pm.network.embedding_net.named_parameters():
                        if "transformer.encoder.layers.7" in name:
                            if param.grad is not None:
                                print(f"DEBUG GRAD NORM {name}: {param.grad.norm().item()}")
                            else:
                                print(f"DEBUG GRAD {name}: None")
                            break
