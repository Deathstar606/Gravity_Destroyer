...
def autocomplete_model_kwargs(
    ...
    if data_sample is not None:
        ...
        if model_kwargs["embedding_type"].lower() == "denseresidualnet":
            ...
            if "tokenizer_kwargs" in model_kwargs["embedding_kwargs"]:
                ...
                if (
                    ...
                    model_kwargs["embedding_kwargs"]["tokenizer_kwargs"]["context_features"] = (
                        2 + num_detectors
                    )
                    model_kwargs["embedding_kwargs"]["tokenizer_kwargs"]["num_blocks"] = (
                        num_detectors
                    )
        ...
        if model_kwargs["posterior_model_type"] == "pretraining":
            ...
            # TODO: get GNPE to work with transformer
            ...
            if gnpe:
                ...
                # Beyond-GR pipeline:
                # context = transformer embedding + conditioning variables
                if len(data_sample) >= 3 and data_sample[2].ndim == 1:
                    conditioning_dim = len(data_sample[2])
                    print("\n========== AUTOCOMPLETE ==========")
                    print("Posterior input_dim:", len(data_sample[0]))
                    print("Embedding context_dim:", context_dim)
                    print("Conditioning dim:", len(data_sample[2]))
                    print(
                        "Final context_dim:",
                        context_dim + len(data_sample[2]),
                    )
                    print("===============================\n")
                    model_kwargs["posterior_kwargs"]["context_dim"] = (
